...
build:
	docker build -t gyrosofwar/muslrust .
run:
	docker run -v $$PWD/test:/volume  -w /volume -it gyrosofwar/muslrust /bin/bash
...
clean-docker:
	docker images gyrosofwar/muslrust -q | xargs -r docker rmi -f
clean-lock:
